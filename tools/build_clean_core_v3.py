...
def source_bonus(source: SourceCatalog) -> float:
    ...
    if "facs" in text or "synthetic" in text or "cafca" in text: return .08
    return 0.0
    ...
    try:
        for source in sources:
            for entry in source.entries:
                ...
                c=Candidate(source,entry,profile,cell,yaw,pitch,preliminary_score(source,entry,profile),tier,decode_vector(entry.get("shape")))
        ...
        # Reserve the neutral background floor before general density fill.
        for profile, needed in BACKGROUND_MINIMUMS.items():
            cells_for_profile=sorted(
                [cell for p,cell in selected_by_group if p==profile and selected_by_group[(p,cell)]],
                key=lambda x:tuple(map(int,x.split(":"))),
            )
            round_index=0
            while sum(1 for c in selected if c.profile.name==profile)<needed:
                added=False
                for cell in cells_for_profile:
                    items=selected_by_group[(profile,cell)]
                    if round_index<len(items):
                        c=items[round_index]
                        if c.key not in used: selected.append(c);used.add(c.key);added=True
                        if sum(1 for x in selected if x.profile.name==profile)>=needed: break
                if not added: break
                round_index+=1
        ...
        if len(selected)<args.target_total:
            overflow=sorted(
                (c for (profile,_),items in evaluated.items() if profile in BACKGROUND_PRIORITY for c in items if c.key not in used),
        ...
        for p,required in BACKGROUND_MINIMUMS.items():
            if background_counts[p]<required: gate_failures.append(f"{p}: {background_counts[p]:,} < required {required:,}")
        for p,required in BACKGROUND_POSE_CELL_MINIMUMS.items():
            if background_cells[p]<required: gate_failures.append(f"{p} pose cells: {background_cells[p]:,} < required {required:,}")
        ...
        def append(payload:bytes)->tuple[str,int,int]:
            ...
            if handle is None or (pack_size and pack_size+len(payload)>PACK_TARGET_BYTES):
                ...
                name=f"clean_v3_faces_{pack_index:05d}.bin";pack_index+=1;pack_size=0;handle=(packs/name).open("wb")
        ...
        for cell,items in sorted(output_entries.items(),key=lambda item:tuple(map(int,item[0].split(":")))):
            ...
            for start in range(0,len(items),SHARD_ENTRY_LIMIT):
                name=f"clean_v3_yaw_{pose_token(yaw)}_pitch_{pose_token(pitch)}_{start//SHARD_ENTRY_LIMIT:03d}.json"
