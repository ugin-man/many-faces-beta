...
const report={testedCommit:process.env.GITHUB_SHA||null,baselineCommit:process.env.ASTRA_BASELINE_COMMIT||null,input:'same three public photographs, native virtual camera',physicalCameraVerified:false,privateUserVideoUsed:false,measurement:'ABBA; fresh browser processes; 20-second count deltas after warmup',resourceMeaning:'Same-origin Window and Worker Resource Timing entries initiated and completed within the measured interval. transferSize is browser-reported transfer including its header accounting; it is not a packet capture. Zero transferSize with a positive decodedBodySize identifies locally served cached resources in this test.',trials:[],passed:false};
...
async function finish(scope){return scope.evaluate(()=>{const end=performance.now();return performance.getEntriesByType('resource').filter(e=>e.startTime>=globalThis.__astraResourceBegin&&e.responseEnd<=end).map(e=>({name:e.name,startTime:e.startTime,responseEnd:e.responseEnd,transferSize:e.transferSize,encodedBodySize:e.encodedBodySize,decodedBodySize:e.decodedBodySize}));});}
async function trial(variant,index){
  ...
  try{
    ...
    const start=await state(page),startAt=performance.now();
    ...
    const end=await state(page),seconds=(performance.now()-startAt)/1000;
...
try{
  ...
  report.shardResponseImprovement=report.summary.shardResponses.after<report.summary.shardResponses.before;
