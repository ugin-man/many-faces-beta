#!/usr/bin/env python3
"""Curate high-quality isolated single-factor faces for Clean Core v2."""
...
from clean_core_policy_v2 import BLEND_KEYS, POLICY_VERSION, classify_clean_profile
...
def main() -> int:
    ...
    reasons: Counter[str] = Counter(); profiles: Counter[str] = Counter(); pose_cells: Counter[str] = Counter()
    ...
    with vision.FaceLandmarker.create_from_options(options) as detector:
        for index, row in enumerate(rows, start=1):
            ...
            try:
                ...
                if profile is None: reasons["mixed_or_unsupported_state"] += 1; continue
                ...
                output_row.update({
                    ...
                    "clean_profile": profile.name, "clean_group": profile.group,
                ...
                accepted_rows.append(output_row); profiles[profile.name] += 1
    ...
    report: dict[str, Any] = {
        ...
        "reasons": dict(reasons), "profiles": dict(sorted(profiles.items())), "poseCells": len(pose_cells),
    }
